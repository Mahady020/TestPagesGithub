...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    if(isset($_POST["iptal"]))
    {
        ...
        $sil_query=$bdconnection->prepare("DELETE FROM ramada_musteri_tablo where musteri_id=?");
        $sil=$sil_query->execute(array($hidden_kod));
        if($sil)
        {
            echo "<script>
            alert('Rezervasyonunuz Başarıyla Iptal Edildi');
            window.location.href = 'http://localhost/gezi_tanitim_turizm_herberi/web_site/ramada_otel_anasayfa_html.php';
            </script>";
            exit();
        }
    }
    if(isset($_POST["gunceleme"]))
    {
        if(isset($_POST["hidden_kod"]))
            $hidden_kod=$_POST["hidden_kod"];
        $call_query= $bdconnection->query("SELECT * FROM ramada_musteri_tablo where musteri_id=$hidden_kod;");
        ...
        <section>
            <div class="dash-board">
                ...
                <div class="main-dash-content">
                    <form action="ramada_otel_kontrol_update_.php" method="post">
