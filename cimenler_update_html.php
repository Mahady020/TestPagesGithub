...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
    if(isset($_POST["iptal"]))
    {
        ...
        $call_query= $bdconnection->query("SELECT * FROM cimen_musteri_tablo ORDER BY musteri_id DESC ");
        $record=$call_query->fetch();
        $no=$record["musteri_id"];



        /*****************************************************************deleting data from database */
        $sil_query=$bdconnection->prepare("DELETE FROM cimen_musteri_tablo where musteri_id=?");
        $sil=$sil_query->execute(array($no));
        if($sil)
        {
            echo "<script>
            alert('Rezervasyonunuz Başarıyla Iptal Edildi');
            window.location.href = 'http://localhost/gezi_tanitim_turizm_herberi/web_site/cimenler_otel_anasayfa.html.php';
            </script>";
            exit();
        }
    }
    ...
    ?>  
       ...
       <!DOCTYPE html>
        ...
        <head>
            ...
            <link rel="stylesheet" href="css/cimenler_update_html.css">
        ...
        <section>
            ...
            <div class="side">
                ...
                <div class="main-dash-content">
                    <form action="cimenler_update_.php" method="post">
